...
La tâche consiste à prédire le coefficient de clustering global du graphe $\bar{C} = \frac{1}{N}\sum_{i=1}^N C_i$ à partir de sa structure. Ce coefficient, variant entre 0 et 1, quantifie l'organisation spatiale cellulaire.
...
\hline
GCN & 0.0198 ± 0.0021 & 250K \\
DeepSets & 0.0145 ± 0.0019 & 280K \\
EGNN & 0.0137 ± 0.0017 & 800K \\
GAT & \textbf{0.0118 ± 0.0014} & 320K \\
\hline
\end{tabular}
\end{center}
...
\textbf{Observations :}
\begin{itemize}
    \item GAT obtient les meilleures performances (MSE = 0.0118), démontrant l'efficacité du mécanisme d'attention
    \item DeepSets atteint de bons résultats (MSE = 0.0145) malgré une architecture plus simple
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Plage clustering} & \textbf{MSE} & \textbf{MAE} \\
\hline
Faible ($[0, 0.33]$) & 0.0082 & 0.063 \\
Modéré ($[0.33, 0.67]$) & 0.0071 & 0.059 \\
Fort ($[0.67, 1.0]$) & 0.0118 & 0.079 \\
\hline
\textbf{Moyenne} & \textbf{0.0089} & \textbf{0.067} \\
\hline
\end{tabular}
\end{center}
...
\begin{tabular}{|l|c|}
\hline
\textbf{Condition} & \textbf{MSE} \\
\hline
Complet (3D + volume) & \textbf{0.0089} \\
Sans positions 3D & 0.0356 (+×4) \\
Sans volume & 0.0102 (+15\%) \\
Positions 2D seulement & 0.0178 (+×2) \\
\hline
\end{tabular}
\end{center>
...
\begin{itemize}
    ...
    \item Les positions seules atteignent des performances très élevées (MSE = 0.0102), confirmant que l'information spatiale domine
\end{itemize}
...
\textbf{Comparaison :}
\begin{itemize}
    \item EGNN complet (équivariant) : MSE = 0.0089
    \item EGNN sans mise à jour coordonnées (messages invariants mais pas de propagation géométrique) : MSE = 0.0124
    \item GNN utilisant coordonnées brutes comme features (non-équivariant) : MSE = 0.0245
\end{itemize}
