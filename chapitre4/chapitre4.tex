...
\textbf{Régression du nombre de parents (tâche auxiliaire) :}
Sur les données synthétiques, nous entraînons les modèles à prédire le nombre de parents du processus de Matérn — une variable continue caractérisant le degré d'agrégation spatiale. Ce paramètre $\lambda_{\text{parent}}$ contrôle le nombre de centres de clusters générés lors de la simulation du processus ponctuel de Matérn. Pour les organoïdes générés par processus de Poisson homogène, on conserve le nombre de points, tandis que pour les processus de Matérn avec forte agrégation, $\lambda_{\text{parent}}$ est faible (nombre de centres de clusters faible). Cette tâche de régression sert de pré-entraînement auto-supervisé : le modèle apprend à extraire des représentations géométriques fines des patterns spatiaux cellulaires, représentations qui se transfèrent ensuite efficacement à la classification de phénotypes réels. L'erreur quadratique moyenne (MSE) sert de métrique d'évaluation pour cette tâche.
...
\subsubsection{Calcul du nombre de parents pour la régression}

Une fois le graphe construit par tessellation de Voronoï, nous utilisons le nombre de parents $\lambda_{\text{parent}}$ du processus de Matérn, qui sert de label pour la tâche de régression sur données synthétiques.

\textbf{Définition et utilisation :}

Le paramètre $\lambda_{\text{parent}}$ du processus de Matérn contrôle directement le nombre de centres de clusters (points parents) générés sur la sphère. Ce paramètre est connu par construction lors de la génération synthétique et sert de label de régression.

\textbf{Relation avec le continuum Poisson-Matérn :}

Le nombre de parents quantifie directement le degré d'agrégation spatiale lors de la génération :

\begin{itemize}
    \item \textbf{Poisson homogène} : $\lambda_{\text{parent}} = 0$ (aucun centre de cluster, distribution aléatoire uniforme)
    
    \item \textbf{Matérn faible clustering} : $\lambda_{\text{parent}}$ élevé (nombreux centres de clusters, chacun a peu de points)
    
    \item \textbf{Matérn fort clustering} : $\lambda_{\text{parent}}$ faible (peu de centres de clusters, chacun a beaucoup de points)
    
    \item \textbf{Continuum intermédiaire} : En variant $\lambda_{\text{parent}}$ continûment, nous générons des patterns avec des degrés d'agrégation variables, couvrant le spectre complet des architectures spatiales
\end{itemize}

\textbf{Propriété clé :} Le nombre de parents $\lambda_{\text{parent}}$ est connu \textit{a priori} lors de la génération du processus ponctuel. Il contrôle directement les corrélations spatiales induites dans le graphe résultant, offrant ainsi une variable cible univoque et continue pour la tâche de régression supervisée, idéale pour apprendre des représentations des patterns spatiaux cellulaires.
...
\textbf{Motivation :}
Exploite pleinement la géométrie 3D et garantit l'équivariance E(3).
